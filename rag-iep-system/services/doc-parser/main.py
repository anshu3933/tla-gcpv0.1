...
CHUNK_SIZES = {"en": 400, "fr": 200, "es": 300, "default": 400}


@asynccontextmanager
async def lifespan(app: FastAPI):
    # Startup
    app.state.tracer = setup_telemetry("doc-parser")
        ...
        with blob.open("rb") as docx_file:
            with zipfile.ZipFile(docx_file) as z:
                with z.open("word/document.xml") as xml:
                    ...
                    context = etree.iterparse(xml, events=("end",), tag=f"{ns}p")
    ...
    async def process_document(self, bucket_name: str, blob_name: str):
        ...
        for idx, chunk in enumerate(chunks):
            ...
            future = self.publisher.publish(
                topic_path, json.dumps(chunk_data).encode("utf-8")
            )
        ...
        logger.info(
            "Document processed", doc_id=doc_id, chunks=len(chunks), language=language
        )
