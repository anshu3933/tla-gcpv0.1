...
from shared.telemetry import setup_telemetry, instrument_fastapi, create_span, get_trace_context
...
async def create_upload_url(
    ...
    try:
        ...
        url = blob.generate_signed_url(
            ...
            headers={"x-goog-if-generation-match": "0"},  # Prevent overwrites
    ...
    try:
        with create_span("query_rag", {
            ...
            with create_span("embed_question") as embed_span:
                ...
                embed_span.set_attribute("embedding_dimensions", len(question_embedding[0].values))
            ...
            with create_span("vector_search") as search_span:
                ...
                search_span.set_attribute("num_results", len(response[0]) if response[0] else 0)
            ...
            if not response[0] or len(response[0]) == 0:
                return QueryResponse(
                    ...
                    sources=[],
                    prompt_version=app.state.prompt_template["version"]
                )
            ...
            # 5. Generate response
            prompt = app.state.prompt_template["template"].format(
                context=context,
            ...
            async def generate():
                with create_span("generate_response") as gen_span:
                    ...
                    final_data = {
                        "done": True,
                        "sources": [{"uri": c["sourceUri"], "score": c["relevance_score"]} 
                                  for c in chunks],
                        "prompt_version": app.state.prompt_template["version"]
                    }
                    ...
                    cost_micro_usd = estimated_tokens * 0.125  # Example pricing
...
async def ready():
    try:
        # Check Firestore
        firestore_client.collection("_health").document("check").set({"ts": datetime.utcnow()})
        # Check models loaded
